...
window.addEventListener('DOMContentLoaded', () => {
  ...
  auth.onAuthStateChanged(async user => {
    if (user) {
      ...
      try {
        // Fix: Wait for profile/admin status before starting any data reads
        await loadProfile(user.uid);
        hideSplash();
        startApp();
        
        // Fix: If user is already on admin tab, trigger the load now that auth is ready
        if (activeTab === 'admin') loadAdminDashboard();
      } catch(e){
        hideSplash();
        const msg = e && e.message ? e.message : 'Check your connection and try again.';
        showErr(msg, ()=>location.reload());
      }
    } else {
      hideSplash();
      showAuth('login');
...
function goTab(tab){
  ...
  if (typeof goTabExtras === 'function') goTabExtras(tab);
}
...
// Store both the callback AND the query ref for each listener
// so .off() is always called on the exact same ref it was attached to.
let feedListenerRef      = null;
let feedChangeWatcherRef = null;
let lfListenerRef        = null;
let rpListenerRef        = null;

function detachListeners(){
  if (feedListener      && feedListenerRef)      { feedListenerRef.off('value', feedListener);           feedListener=null;      feedListenerRef=null; }
  if (feedChangeWatcher && feedChangeWatcherRef) { feedChangeWatcherRef.off('child_added', feedChangeWatcher); feedChangeWatcher=null; feedChangeWatcherRef=null; }
  if (lfListener        && lfListenerRef)        { lfListenerRef.off('value', lfListener);               lfListener=null;        lfListenerRef=null; }
  if (rpListener        && rpListenerRef)        { rpListenerRef.off('value', rpListener);               rpListener=null;        rpListenerRef=null; }
}
...
function attachFeedListener(){
  ...
  // Detach old realtime watcher if any — must use the same ref it was attached to
  if (feedListener && feedListenerRef){ feedListenerRef.off('value', feedListener); feedListener=null; feedListenerRef=null; }
  if (feedChangeWatcher && feedChangeWatcherRef){ feedChangeWatcherRef.off('child_added', feedChangeWatcher); feedChangeWatcher=null; feedChangeWatcherRef=null; }
  ...
  feedListenerRef = db.ref('posts').orderByChild('t');
  feedListener = feedListenerRef.on('value', snap => {
      ...
      // onError callback — detach before retrying to avoid stacked listeners
      if (feedListener && feedListenerRef){ feedListenerRef.off('value', feedListener); feedListener=null; feedListenerRef=null; }
      el('skeletons')?.classList.add('hidden');
      feedRetryCount++;
      if (feedRetryCount <= 3){
...
function startFeedChangeWatcher(){
  ...
  feedChangeWatcherRef = db.ref('posts').orderByChild('t').startAfter(feedNewestTs > 0 ? feedNewestTs : Date.now());
  feedChangeWatcher = feedChangeWatcherRef.on('child_added', snap => {
      const p = snap.val();
      if (!p || !p.body) return;
...
function attachLFListener(){
  ...
  lfListenerRef = db.ref('posts').orderByChild('t');
  lfListener = lfListenerRef.on('value', snap=>{
    if(!list) return;
    list.innerHTML='';
    let posts=[];
    snap.forEach(c=>{
...
function attachRPListener(){
  ...
  rpListenerRef = db.ref('posts').orderByChild('t');
  rpListener = rpListenerRef.on('value', snap=>{
    if(!list) return;
    list.innerHTML='';
    let posts=[];
    snap.forEach(c=>{
...
function buildCard(p){
  ...
  const locDataAttr = esc((p.location||p.authorBrgy||'').replace(/'/g,'&#39;'));
  return `<article class="pcard${p.escalated?' escalated-card':''}" data-location="${locDataAttr}" id="pc-${safeKey}" aria-label="Post by ${esc(p.authorName)}">
...
async function loadAdminDashboard(){
  ...
  try {
    // Fix: Use allSettled so one blocked folder doesn't crash the whole screen
    ...
    const postsSnap = getData(0);
    const usersSnap = getData(1);
    const reportsSnap = getData(2);
    const onlineSnap = getData(3);

    // Update UI safely—if data is missing, show 'N/A' instead of an error
    setText('aTotalPosts', postsSnap ? postsSnap.numChildren() : 'N/A');
    setText('aTotalUsers', usersSnap ? usersSnap.numChildren() : 'N/A');
    setText('aReports', reportsSnap ? reportsSnap.numChildren() : 'N/A');

    const now = Date.now();
    let online = 0;
    if (onlineSnap) {
      onlineSnap.forEach(c => {
        if (now - c.val().t < 120000) online++;
      });
    }
    setText('aOnline', onlineSnap ? online : 'N/A');

    adminTab(adminSection);
  } catch(e) {
    console.error("Dashboard stats error:", e);
    toast("Some stats could not be loaded.", "warn");
  }
}
...
// ── EXTRA goTab FEATURE INITS (non-patch, safe) ───────────────────
// Called after the main goTab logic via the DOMContentLoaded hook below.
function goTabExtras(tab){
  if(tab==='home'){
    setTimeout(()=>{
      injectDistances();
      addChatButtons();
