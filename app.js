...
async function uploadAvatar(input){
  ...
  try {
    ...
    showProgress('Uploading photo...', 20);
    ...
    await new Promise((resolve, reject) => {
      task.on('state_changed',
        snap => {
          const pct = 20 + Math.round((snap.bytesTransferred / snap.totalBytes) * 75);
  ...
  finally { hideProgress(); input.value=''; }
}
...
async function submitPost(){
  ...
  try {
    if (postImgFile){
      ...
      const compressed = await compressImage(postImgFile, 800, 800, 0.65);
      showProgress('Uploading image...', 25);
      ...
      await new Promise((resolve, reject) => {
        task.on('state_changed',
          snap => {
            const pct = 25 + Math.round((snap.bytesTransferred / snap.totalBytes) * 65);
