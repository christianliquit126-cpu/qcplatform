...
function loadNearbyPosts(){
  ...
  if(!container) return;
  db.ref('posts').orderByChild('feed').equalTo('feed').once('value').then(snap=>{
    const posts=[];
    snap.forEach(c=>{ const p={key:c.key,...c.val()}; const bc=BRGY_COORDS_EXT[p.location]||(typeof BRGY_COORDS!=='undefined'?BRGY_COORDS[p.location]:null); if(bc){const km=getDistKm(userLat,userLng,bc.lat,bc.lng);if(km!==null&&km<10) posts.push({...p,distKm:km});} });
...
async function updateHeroStats(){
  try {
    const [postsSnap,onlineSnap]=await Promise.all([
      db.ref('posts').orderByChild('feed').equalTo('feed').once('value'),
